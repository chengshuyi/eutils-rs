...
mod tests {
    use super::*;
    #[test]
    fn test_ksyms_try_from() {
        assert_eq!(Kallsyms::try_from("/proc/kallsyms").is_ok(), true);
        assert_eq!(Kallsyms::try_from("/3124/2123").is_ok(), false);
    }

    #[test]
    fn test_ksyms_has_sym() {
        let ksym = Kallsyms::try_from("/proc/kallsyms").unwrap();
        assert_eq!(ksym.has_sym("schedule"), true);
        assert_eq!(ksym.has_sym("5813478"), false);
    }
}
